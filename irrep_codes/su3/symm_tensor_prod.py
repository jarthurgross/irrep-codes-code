...
class SL3CRep(abc.ABC):
    ...
    @abc.abstractmethod
    def get_diagonal_op(self, positive_index: int, negative_index: int) -> np.ndarray:
...
class StandardSL3CRep(SL3CRep):
    ...
    def get_diagonal_op(self, positive_index: int, negative_index: int) -> np.ndarray:
        return np.outer(
            self.basis[positive_index], self.basis[positive_index]
...
def get_product_rule_lie_algebra_representative(
    ...
    """
    assert all(
        representative.shape[0] == representative.shape[1]
        and len(representative.shape) == 2
        for representative in representatives
    )
    identities = [
        np.eye(representative.shape[0], dtype=representative.dtype)
        for representative in representatives
    ]
    return sum(
        reduce(
...
class TensorProductSL3CRep(SL3CRep):
    ...
    def get_diagonal_op(
        ...
        representatives = [
            rep.get_diagonal_op(positive_index, negative_index)
...
def get_iterative_raising_operator_applications(
    ...
    while most_recently_generated_vectors:
        ...
        most_recently_applied_raising_sequences = actively_applied_raising_sequences
...
def make_projector_out_of_image_vectors(
    ...
    return np.array(
        [
            left_singular_column_vectors[:, idx]
...
def symmetrize_trit_vector_slow_unnorm(
    ...
    canonical_ordering = sum(
        [multiplicity * [idx] for idx, multiplicity in enumerate(multiplicities)],
        [],
    )
    return sum(
